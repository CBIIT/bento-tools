...
const DashboardFunctions = (props) => {
  // Defining props.
  const {
    store,
    storeKey,
 ...
 */
  ...
  const getSubjectIds = () => getState().filteredSubjectIds;
 ...
 */
  ...
  function clearSectionSort(groupName) {
    store.dispatch({
 ...
 */
  ...
  function localSearch(searchcriteria) {
 ...
 * @return distpatcher
 */
  function resetGroupSelections(payload) {
    return () => {
 ...
 */
  ...
  function fetchDataForDashboardTab(
 ...
 * @return {json}
 */
  async function fetchAllFileIDsForSelectAll(fileCount = 100000) {
 ...
 * @return {json}
 */
  async function fetchAllFileIDs(fileCount = 100000, selectedIds = []) {
    let filesIds = [];
  ...
  const getFilesCount = () => getState().stats.numberOfFiles;
 ...
 * @return distpatcher
 */
  async function setSingleFilter(payload) {
 ...
 */
  ...
  async function singleCheckBox(payload) {
 ...
 * @return distpatcher
 */
  function toggleCheckBox(payload) {
    return () => {
      const currentAllFilterVariables = payload === {} ? allFilters : createFilterVariables(payload);
      // For performance issue we are using initial dasboardquery instead of fitered for empty filters
      if (_.isEqual(currentAllFilterVariables, allFilters())) {
 ...
 */
  ...
  function setDashboardTableLoading() {
 ...
 */
  ...
  function sortSection(groupName, sortBy) {
    store.dispatch({
 ...
 * @return {json}
 */
  function updateFilteredAPIDataIntoCheckBoxData(data, facetSearchDataFromConfig) {
    return (
  ...
  function getCountForAddAllFilesModal() {
 ...
 */
  ...
  async function getFileNamesByFileIds(fileIds) {
    const data = await client
      .query({
 ...
 * @return {json}
 */
  async function tableHasSelections() {
    let selectedRowInfo = [];
    let filteredIds = [];
 ...
 */
  ...
  function getTableRowSelectionEvent() {
    const currentState = getState();
  ...
  const getDashboard = () => getState();

  return {
    getState,
    shouldFetchDataForDashboardTabDataTable,
    getStatInit,
    getFilteredStat,
    removeEmptySubjectsFromDonutData,
    getWidgetsInitData,
    allFilters,
    ...
    clearAllFilters,
    localSearch,
    toggleCheckBoxWithAPIAction,
    resetGroupSelections,
    querySwitch,
    getQueryAndDefaultSort,
    fetchDataForDashboardTab,
    transformCasesFileIdsToFiles,
    transformfileIdsToFiles,
    fetchAllFileIDsForSelectAll,
    getFileIDs,
    filterOutFileIds,
    fetchAllFileIDs,
    getFilesCount,
